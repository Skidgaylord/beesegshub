function GetUi() 
    if getgenv().Tvk then
    if game.CoreGui:FindFirstChild("Skid Hub GUI") then
        for a, v in ipairs(game.CoreGui:GetChildren()) do
            if v.Name == "Skid Hub GUI" then
                v:Destroy()
            end
        end
    end
...
function ReadCustomUISetting()
    local l, m =
        ...
        function()
            local j = game:GetService("HttpService")
            if not isfolder("Skid Hub") then
                makefolder("Skid Hub")
            end
            local n = j:JSONDecode(readfile("Skid Hub/" .. k))
...
function t.CreateMain(H)
    ...
    function u.ReloadMain(v)
        ...
        if
            v ~= "" and type(v) == "string" and string.find(v:lower(), ".webm") and
                pcall(
                    function()
                        writefile("skidhub.webm", syn.request({Url = v}).Body)
                    end
                )
         then
            wait(.25)
            local at = isfile("skidhub.webm")
            wait(.25)
            if at then
    ...
    local X = tostring(u.Getcolor(getgenv().UIColor["Title Text Color"])[3])
    local g = V .. "," .. W .. "," .. X
    ad.Text = '<font color="rgb(' .. g .. ')">Skid Hub</font> ' .. getgenv().MainDesc
    table.insert(
        c["Title Text Color"],
        function()
            aa.ImageColor3 = getgenv().UIColor["Title Text Color"]
    ...
    if
        au ~= "" and type(au) == "string" and string.find(au:lower(), ".webm") and
            pcall(
                function()
                    writefile("skidhub.webm", syn.request({Url = au}).Body)
                end
            )
     then
        wait(.25)
        ...
        if at then
            ...
            ar.Video = getsynasset("skidhub.webm")
            ar.Looped = true
            ar:Play()
            wait(.5)
            delfile("skidhub.webm")
        end
    else
            ...
            if Settings.Farm and HumanoidRootPart then
                ...
                if token then
                    if getgenv().IsValidTokenPos(token, TempField,FieldPosIn) and getgenv().IsToken(token) and not getgenv().IsTokenBlacklist(token) and Settings.Farm and ValidFarm() then
                     ...
                     if TempField ~= "Skid Hub Winning Field" and plr.Character:FindFirstChild("HumanoidRootPart") then 
                    ...
                    embeds = {
                        {
                            ...
                            author = {
                                name = "Skid Hub Bee Swarm Simulator Bug Reporter"
                            }
                        }
                    }
                }
